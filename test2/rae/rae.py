...
class RecursiveAutoencoder(object):
    ...
    def backward(self, root_node, total_grad, delta_parent=None):
        ...
        self.__backward(root_node, total_grad, delta_parent_out)
  
    def __backward(self, node, total_grad, delta_parent_out):
        '''Backward pass of training recursive autoencoder using backpropagation
        through structures.
        ...
        if isinstance(node, InternalNode):
            ...
            total_grad.gradWo1 += dot(delta_out1, node.p.T)
            total_grad.gradWo2 += dot(delta_out2, node.p.T)
            total_grad.gradbo1 += delta_out1
            total_grad.gradbo2 += delta_out2
            ...
            self.__backward(node.left_child, total_grad, delta_parent_out_left)
      
            delta_parent_out_right = dot(self.Wi2.T, delta_parent) - node.y2_minus_c2
            self.__backward(node.right_child, total_grad, delta_parent_out_right)
